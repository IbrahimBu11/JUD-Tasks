...
public class lb_Bird : MonoBehaviour {
	...
	void PauseBird(){
		if(!dead){
			...
			anim.speed = 0;
			if(!rb.isKinematic){originalVelocity = rb.velocity;}
			rb.isKinematic = true;
			ad.Stop ();
			paused = true;
		}
	...
	IEnumerator FlyToTarget(Vector3 target){
		if(Random.value < .5){
			ad.PlayOneShot (flyAway1,.1f);
		}else{
			ad.PlayOneShot (flyAway2,.1f);
		}
		flying = true;
		landing = false;
		onGround = false;
		...
		rb.AddForce((transform.forward * 50.0f*controller.birdScale)+(transform.up * 100.0f*controller.birdScale));
		float t = 0.0f;
		while (t<1.0f){
			if(!paused){
				t+= Time.deltaTime;
		...
		if(vectorDirectionToTarget.y>.5f){
			...
			while(vectorDirectionToTarget.y>.5f){
				...
				rb.AddForce(transform.forward * 70.0f*controller.birdScale * Time.deltaTime);
				...
				if (Physics.Raycast(transform.position,-Vector3.up,out hit,0.15f*controller.birdScale) && rb.velocity.y < 0){
		...
		while(true){
			if(!paused){
				...
				if (Physics.Raycast(transform.position,-Vector3.up,0.15f*controller.birdScale) && rb.velocity.y < 0){
					rb.velocity = new Vector3(rb.velocity.x, 0.0f,rb.velocity.z);
				}
				if (Physics.Raycast(transform.position,Vector3.up,out hit,0.15f*controller.birdScale) && rb.velocity.y > 0){
					//if the bird is going to collide with something overhead zero out vertical velocity
				...
				rb.AddForce(transform.forward * flyingForce * Time.deltaTime);
				distanceToTarget = Vector3.Distance (transform.position,target);
	...
	void PlaySong(){
		if (!dead){
			if(Random.value < .5){
				ad.PlayOneShot (song1,1);
			}
            else
            {
                ad.PlayOneShot(song2, 1);
